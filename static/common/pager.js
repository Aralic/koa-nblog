 ...
 *  pContainer:   {Element|String}    // 参数：分页组件容器或其选择器(必选)。
     ...
     stopEvt {Boolean}              // 参数：当分页标签被点击时，是否阻断事件(可选，默认为false)。
...
Pager.prototype.createHtml = function () {
    ...
    if (curIndex < (Math.floor(pMaxCount/2)+1)) {
        for (var i = 0; i < pMaxCount; i++) {
            var pageId = i + 1;
            if (pageId === curIndex) {
                str += '<li class="active"><a data-page="'+pageId+'">'+ pageId +'</a></li>';
            }
            else {
                str += '<li><a data-page="'+pageId+'">'+ pageId +'</a></li>';
            }
        }
    }
    // 右
